...
async function generateSearchQueries(topic: string, n: number) {
  const { text: completion } = await generateText({
    ...
    prompt: `I'm writing a research report on ${topic} and need help coming up with diverse search queries.
...
export function createResearcherAgent() {
  const agent: SlackAgent = {
    ...
    description:
      "Use this agent to create a research report.  It is best used to perform deep research on a topic.",
    handle: "researcher-agent",
    generateResponse: async ({ content: topic }) => researcher(topic),
  };
  return agent;
}
